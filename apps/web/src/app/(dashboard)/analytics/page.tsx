...
function Skeleton({ className }: { className?: string }) {
  return <div className={`animate-pulse bg-[hsl(var(--muted))] rounded-lg ${className ?? ""}`} />;
}
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <MetricCard title="Task Completion" value={`${a.taskCompletionRate}%`} icon={<CheckSquare className="h-5 w-5" />} trend={{ value: 0, label: "from completed tasks" }} />
          ...
          <MetricCard title="Tasks Done" value={a.totalTasksCompleted} icon={<Award className="h-5 w-5" />} description={`${a.completedMilestones} milestones`} />
        </div>
      ) : (
        <p className="text-sm text-[hsl(var(--text-secondary))]">Could not load analytics. Please try again.</p>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="pt-0">
              {a.progressOverTime.length === 0 ? (
                <p className="text-xs text-[hsl(var(--text-secondary))] py-8 text-center">No data yet — complete some tasks to see progress.</p>
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={a.progressOverTime} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="planned" stroke="#a78bfa" strokeWidth={2} strokeDasharray="4 4" name="Total Tasks" dot={false} />
                    <Line type="monotone" dataKey="actual" stroke="#6b21a8" strokeWidth={2} name="Completed" dot={{ r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="pt-0 flex items-center justify-center gap-6">
              {taskStatusData.every(d => d.value === 0) ? (
                <p className="text-xs text-[hsl(var(--text-secondary))] py-8">No task data yet.</p>
              ) : (
                <>
          ...
          <Card>
            ...
            <CardContent className="pt-0 space-y-3">
              {a.categoryProgress.length === 0 ? (
                <p className="text-xs text-[hsl(var(--text-secondary))] py-4">Create missions to see category progress.</p>
              ) : (
      ...
      {a && a.recommendedFocusAreas.length > 0 && (
