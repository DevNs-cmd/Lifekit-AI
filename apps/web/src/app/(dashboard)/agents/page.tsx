...
import { getAgents } from "@/lib/api/ai";
...
function AgentCardSkeleton() {
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 animate-pulse lg:col-span-2 min-h-[400px]">
      <div className="h-12 w-12 rounded-2xl bg-[hsl(var(--muted))] mb-4" />
      <div className="h-5 w-32 rounded bg-[hsl(var(--muted))] mb-2" />
      <div className="h-3 w-24 rounded bg-[hsl(var(--muted))] mb-4" />
      <div className="space-y-2">
        {[1, 2, 3].map(i => <div key={i} className="h-3 rounded bg-[hsl(var(--muted))]" />)}
      </div>
    </div>
  );
}
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-[hsl(var(--primary))]">
            <Sparkles className="h-3.5 w-3.5" />Specialist intelligence
          </div>
          ...
          <p className="mt-1 text-sm text-[hsl(var(--text-secondary))]">
            Choose a specialist that understands your goals, missions, and context.
          </p>
        </div>
        {!loading && (
          <Badge variant="secondary" className="w-fit px-3 py-1">
            {agents.filter(a => a.isAvailable).length} agents ready
          </Badge>
        )}
      </header>
        ...
        {loading ? (
          ...
          agents.map((agent, index) => {
            ...
            return (
              <Card
                key={agent.id}
                className={`group flex flex-col overflow-hidden hover:border-[hsl(var(--primary))]/30 lg:col-span-2 ${agents.length % 3 === 2 && index === agents.length - 2 ? "lg:col-start-2" : ""}`}
                onClick={() => router.push(`${ROUTES.AGENTS}/${agent.id}`)}
              >
                <CardContent className="flex min-h-[400px] flex-1 flex-col p-5">
                  <div className="mb-4 flex items-start justify-between gap-2">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ring-1 ring-black/[0.04] ${config.bg}`}>
                      <DomainIcon className={`h-6 w-6 ${config.color}`} style={{ color: config.hex }} />
                    </div>
                    <Badge variant={agent.isAvailable ? "success" : "outline"} className="mt-1 shrink-0">
                      {agent.isAvailable ? "Ready" : "Offline"}
                    </Badge>
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="space-y-1.5">
                      {agent.capabilities.slice(0, 3).map(capability => (
                        <li key={capability} className="flex items-start gap-2 text-xs text-[hsl(var(--text-secondary))] dark:text-gray-300">
                          <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-[hsl(var(--primary))]/50" />{capability}
                        </li>
                      ))}
                      {agent.capabilities.length > 3 && (
                        <li className="pl-3.5 text-xs text-[hsl(var(--text-secondary))] dark:text-gray-400">
