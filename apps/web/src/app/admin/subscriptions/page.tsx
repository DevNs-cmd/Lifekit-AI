...
const MOCK_SUBS = [
  { user: "Arjun Sharma",  email: "arjun@example.com",  plan: "plus",       since: "Jan 2024", status: "active" },
  { user: "Rahul Gupta",   email: "rahul@example.com",  plan: "pro",        since: "Feb 2024", status: "active" },
  { user: "Anita Desai",   email: "anita@example.com",  plan: "plus",       since: "Apr 2024", status: "past_due" },
  { user: "Priya Nair",    email: "priya@example.com",  plan: "free",       since: "Mar 2024", status: "active" },
  { user: "Kiran Mehta",   email: "kiran@example.com",  plan: "free",       since: "May 2024", status: "active" },
];

const STATUS_VARIANTS: Record<string, "success" | "warning" | "outline" | "destructive"> = {
  active: "success", past_due: "warning", cancelled: "destructive", trialing: "info" as "outline",
};

const PLAN_VARIANTS: Record<string, "purple" | "success" | "secondary" | "outline"> = {
  pro: "purple", plus: "success", free: "outline",
};

export default function AdminSubscriptionsPage() {
  const planCounts = MOCK_SUBS.reduce((acc, s) => {
            ...
            <tbody className="divide-y divide-[hsl(var(--border))]">
              {MOCK_SUBS.map((s, i) => (
                <tr key={i} className="hover:bg-[hsl(var(--background-subtle))] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <Badge variant={STATUS_VARIANTS[s.status] ?? "outline"} className="capitalize">{s.status.replace("_", " ")}</Badge>
                  </td>
