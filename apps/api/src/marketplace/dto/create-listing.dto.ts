...
import { IsArray, IsBoolean, IsNotEmpty, IsNumber, IsString, Min, MinLength, ValidateNested } from 'class-validator';
import { ListingAvailabilityDto } from './listing-availability.dto';

export class CreateListingDto {
  @ApiProperty({ description: 'Title of the listing', example: 'Productivity Blueprint Course', minLength: 5 })
  @IsString()
  @IsNotEmpty({ message: 'Title is required' })
  @MinLength(5, { message: 'Title must be at least 5 characters long' })
  title!: string;

  @ApiProperty({ description: 'Description of the listing', example: 'A complete guide to mastering time management and daily schedules.', minLength: 10 })
  @IsString()
  ...
  @ApiProperty({ description: 'Availability configurations', type: () => ListingAvailabilityDto })
  @ValidateNested()
